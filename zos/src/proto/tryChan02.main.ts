...
modelSeedPolicy.xFromBit = 40;
modelSeedPolicy.xRunsForward = false;
modelSeedPolicy.xToBit = 126;
modelSeedPolicy.yFromBit = 40;
modelSeedPolicy.yRunsForward = true;
modelSeedPolicy.yToBit = 126;
...
const strategies = [
   ...
   modelSeedStrategyThree, modelSeedStrategyFour
];


const dir = '/Users/jheinnic/Documents/randomArt3/pkFixture4';
const workFactory = new ConcurrentWorkFactory();
const autoIterate = new AutoIterate(asyncScheduler, workFactory);

const keyUuids: string[] = fs.readFileSync(`${dir}/keys.dat`, {encoding: 'utf8'}).split(/\n/);
...
co(function* () {
   ...
   (err: any) => {
      console.error('Canvas recycling coroutine has exited on an error!  No new painting will begin:', err);
   }
);
