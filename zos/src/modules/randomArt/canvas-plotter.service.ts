...
import {count, concatMap, tap, reduce, take} from 'rxjs/operators';
...
export class CanvasPlotter
{
   paintThenEmit(nextTask: CanvasAndPlotModel): Observable<number>
   {
      ...
      // console.log(nextTask: <Observable<Observable<PointMap>>);
      let batchCount = 72;
      nextTask.pointMapBatches.pipe(
         tap((batch: Observable<PointMap>) => {
      ...
      return nextTask.pointMapBatches.pipe(
         take(batchCount - 1), concatMap(
            nextBatch => {
               ...
               return nextBatch.pipe(reduce(
                  (acc: number, nextPointMap: PointMap): number => {
