...
import {StoreArtworkRequest} from '../messages/store-artwork-request.value';
import { IDirectoryUtils } from '../../../infrastructure/lib/directory-utils.interface';
import { CompareGoal } from '../../../apps/modules/features/gene-game/model/card-face.interface';
...
export class CanvasWriter
{
   ...
   constructor(
      private readonly inputChan: Chan<any, StoreArtworkRequest>,
      private readonly compareGoal: CompareGoal;
      ...
      private readonly requirePrivacy: boolean = false)
   {
      ...
      if (requirePrivacy) {
         this.directoryCheck = dirUtils.ensurePrivateDir(this.outputDir);
      } else {
         this.directoryCheck = dirUtils.ensureWritableDir(this.outputDir);
      }
   }
