...
import {PaintActivityModels} from '../store/models/individuals.models';
...
import PaintProcessModel = PaintActivityModels.PaintProcessModel;
...
class CanvasPaintPlanFactory
{
  private static logComputeEvery = 5000;

  constructor()
  {
  }

  calculateCanvasPaintPlan(paintProcess: PaintProcessModel, populationPolicy: PopulationModelPolicy): CanvasPaintPlan
  {
    ...
    const progressTick = CanvasPaintPlanFactory.logComputeEvery / pixelCount;
...
class CanvasPaintPlan
(context: CanvasRenderingContext2D) => Promise<any>
    return function (paintContext: CanvasRenderingContext2D): Promise<any> {
      return new Promise<any>((resolve, reject) => {
        ...
        for (const colorState of colorsAlreadySeen) {
          ...
          for (
            let xx = colorState.xLastSeen,
              yy = colorState.yLastSeen; xx >= 0, xx = nextCellByCell.g)
          {
            if (yy < 0) {
...
factoryMethod(canvas.getContext('2d'))
  .then(console.log)
  .catch(console.error);
