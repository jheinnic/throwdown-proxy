...
export class AssetLease<L, A> implements Unsubscribable
{
    ...
    private readonly _lessorAsset: A)
  {
    ...
    const foo1A = modifyApi.createChainHandler(this._lesseeHandler);
    const foo1B = modifyApi.createChainHandler(this._lessorHandler);
    console.log('Chain handlers:', foo1A, foo1B);

    const foo2A = this._lesseeHandler.addProxyListener((meta) => {
      console.log(meta);
    });
    const foo2B = this._lesseeHandler.addProxyListener((meta) => {
      console.log(meta);
    });
